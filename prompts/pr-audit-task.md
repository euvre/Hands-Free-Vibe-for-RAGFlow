...
1. Read `meta.md`.
2. Full comment inventory: read the gh-recorder snapshot `__HFV_DIR__/lines/gh-store/pr-__PR_NUM__-comments.md` (complete reconciled inventory, refreshed asynchronously every minute; image attachments transcribed inline; every body inlined in full — only >6000-char walls are capped, with a pointer into the sibling `-comments.jsonl`). Fresh = mtime within 10 minutes (`stat -c %Y`). Missing or stale → fall back to the live fetcher `bash __HFV_DIR__/framework/pr-comments-fetch.sh __GITHUB_REPO__ __PR_NUM__`; on its `FATAL: partial fetch` rerun ONCE, and if it still fails write an INCOMPLETE verdict stating that comment collection failed. Never audit from a partial view; never cut a comment listing through `head`/`tail`/`sed -n`.

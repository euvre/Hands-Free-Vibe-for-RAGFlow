...
5. Deliver: one NEW focused commit (never `--amend`, never force-push) with a conventional message matching the failure class (`style: ...` for format, `fix: ...` for behavior, `test: ...` for tests). The repo's committer identity is pre-configured — commit normally. That is where your job ends: the framework pushes your committed HEAD to the fork's `__BRANCH__` after this task ends (this container has no credentials — never `git push`, never touch the outbox). If the remote branch moved meanwhile (`git fetch __FORK_REMOTE__ __BRANCH__ && git status -sb`), `git rebase __FORK_REMOTE__/__BRANCH__` and re-verify quickly so the push stays fast-forward; leave the branch in that committed state either way.

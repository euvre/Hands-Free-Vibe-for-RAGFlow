# PR audit task — dual-check variant (we are the reviewer; Go/Python parity is mandatory)
...
1. Read `meta.md`.
2. Full comment inventory: read the gh-recorder snapshot `__HFV_DIR__/lines/gh-store/pr-__PR_NUM__-comments.md` (complete reconciled inventory, refreshed asynchronously every minute; image attachments transcribed inline; every body inlined in full — only >6000-char walls are capped, with a pointer into the sibling `-comments.jsonl`). Fresh = mtime within 10 minutes (`stat -c %Y`). Missing or stale → `python3 __HFV_DIR__/lines/gh-outbox.py rescan --pr __PR_NUM__`, wait ~90s, re-check; still stale after TWO rescan requests → write an INCOMPLETE verdict stating that comment collection failed. Never audit from a partial view; never cut a comment listing through `head`/`tail`/`sed -n`; no gh calls (this container has no GitHub credentials).
...
- **Dual-implementation parity (binding in this variant)**: when the changed surface has BOTH a Python and a Go implementation in this repo (`api/` / `agent/` / `rag/` ↔ `internal/` — e.g. canvas scheduler, REST routes, SSE emitters, ingestion pipeline), evaluate BOTH sides: whether the same defect class exists on the other side, and whether behavior stays consistent across them — each with `file:line` evidence. First enumerate which side(s) exist (`ls internal/`, `grep` for the mirrored route/mechanism); if the other implementation has no equivalent mechanism, state that explicitly with the basis. A verdict silent on the other implementation is incomplete. The verdict body MUST carry a "**Go/Python parity**" line stating what was checked on each side. A live sibling still carrying the same defect class triggers the parity gate in section 5 (the verdict is at least PROBLEMS).
...
- **Browser-class verification — mandatory in every audit whose change has ANY runtime behavior** (only pure docs/comments/types-only changes are exempt, see below): app services are ALREADY READY (pre-flight; never redo the bring-up). A PARTIAL pre-flight — py api + web READY, go gateway still down — still fully supports the browser path (web proxies /api to py by default): the browser tier proceeds as normal in that case.
  ...
  2. Capture screenshots with the browser MCP's take_screenshot into `__HFV_DIR__/audit/pr-__PR_NUM__/shots/`, numbered descriptive names (`01-scenario.png`, `02-regression.png`, …) — they are your evidence. Reference them from the verdict body with image syntax — `![short alt](shots/01-scenario.png)` — never as backtick-quoted paths: the post stage uploads referenced shots to the fork's asset release and rewrites them to public URLs that render inline in the GitHub comment; a plain-text path renders as dead text there.

  All of this is safe inside this container; anything outside its namespace stays off limits per section 0.
- **No-UI-surface changes (pure backend/CLI/dev-tooling, e.g. `test/benchmark`)**: the browser cannot drive the scenario itself (it can neither trigger transport-level faults nor read CLI reports), so the scenario goes programmatically — an HTTP client / CLI / script walking what the PR claims plus ONE adjacent regression path — AND ADDITIONALLY the browser tier still runs a smoke pass: open the app, log in, walk ONE core page whose backend the PR touches (e.g. the dataset detail page for a retrieval change), with a screenshot as evidence. The browser is never skipped outright while web 9222 is up; the verdict states what each tier covered.
- Pure docs/comments/types-only change: state that verification was de-scoped and why — the ONLY acceptable browser skip.
- Environment genuinely unusable (py api or web down, disk full, provider quota dead): fall back to the unit tiers + code-level evidence, do not retry the bring-up in a loop, and choose INCOMPLETE (not PROBLEMS) so the verdict states the boundary honestly. A go-gateway-only outage is NOT this case — see the browser bullet.
...
- **LGTM** ⇔ no blocker and no major finding, AND the behavior was verified: browser-class e2e with screenshots for every runtime change (for no-UI-surface changes: the programmatic scenario drive PLUS the browser smoke pass), or a legitimately de-scoped pure-non-runtime change. Body starts with `LGTM 🚀`, then a short "What was verified" list: what you ran, what you saw, screenshot references. **Always declare the verification tier in one line**, naming what the browser pass covered; outside the de-scoped case, a missing browser pass reads as an unaudited surface.
- **Parity gate (binding in this variant)**: when the section-3 parity check finds the same defect class still present on the other implementation AND that implementation serves production traffic (wired into the router / reachable by users — verify this with code, never assume), the verdict is at least PROBLEMS with a `major` finding "fix incomplete across implementations" — UNLESS the PR's stated scope is explicitly single-implementation AND the verdict body carries a concrete follow-up (an issue link or an explicit commitment). A sibling implementation still serving the broken behavior to users is never a mere note.

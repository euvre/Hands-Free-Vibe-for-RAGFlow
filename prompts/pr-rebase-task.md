...
7. Deliver: leave the rebased, verified state committed on the branch — nothing else. The framework force-with-lease pushes `__WORKDIR__`'s HEAD to the fork's `__BRANCH__` after this task ends (the lease expectation is this worktree's remote-tracking ref; a refusal means someone else pushed meanwhile — the next round re-evaluates, so just report the state and stop). Never `git push` yourself (this container has no credentials). A rebase left in progress or an unmoved HEAD is read as "nothing to push".

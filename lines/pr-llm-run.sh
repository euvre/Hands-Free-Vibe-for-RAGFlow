...
$ENV_STATUS_SECTION"
  ...
  while true; do
    ...
    # parked-on-rate-limit marker for `hfv ps` / `hfv model`: written as soon
    # as a quota/transient failure is classified — BEFORE the rotation branch,
    # so a run spinning through key rotation also reads as WAIT — and cleared
    # only when the retry loop exits, not between attempts.
  ...
  # retry loop settled (success / give-up): clear the rate-limit marker

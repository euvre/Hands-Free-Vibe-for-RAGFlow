...
CLONE="$RAGFLOW_MAIN"
WTROOT="$CLONE/wt"
TMPL="$(resolve_prompt pr-review-task)"
...
release_worktree() { # <pr-num>
  local wt="$WTROOT/review-$1"   # must match prepare_worktree's review-$num
...
while IFS=$'\t' read -r num branch url mid; do
  ...
  HFV_UNLOCK_FLAG="$DIR/state/pr-unlock-review-$INST.flag" \

#!/usr/bin/env bash
# pr-rerun-line.sh — manual RE-RUN of an existing PR's original task, with the
...
#
# Manual only (hfv pr rerun <pr>); no timer, no collect — the user names the
# PR. One PR per invocation, own lock (pr-rerun.lock), own worktree namespace
# wt/rerun-<n> on the shared clone. Serial with itself only; the per-PR guard
...
CUR_PR=""
trap 'rm -f "$HFV_EXEC_SNAPSHOT"; if [[ -n "${CUR_PR:-}" && -n "${DIR:-}" ]]; then rm -f "$CUR_FILE"; docker rm -f $(docker ps -q --filter "name=-spr-rerun-$CUR_PR") >>"${LOG_DIR:-/dev/null}" 2>&1 || true; fi' EXIT
...
CLONE="$RAGFLOW_MAIN"
WTROOT="$CLONE/wt"
LOCK_FILE="$DIR/pr-rerun.lock"
CUR_FILE="$DIR/.current-rerun"
TMPL="$(resolve_prompt pr-rerun-task)"

mkdir -p "$LOG_DIR" "$WTROOT" "$DIR/scratch"
log() { echo "[$(date +%Y%m%d-%H%M%S)] pr-rerun-line: $*" >> "$LOG_DIR/daemon.log"; }

[[ "${1:-}" == "--single" && "${2:-}" =~ ^[0-9]+$ ]] || {
  echo "usage: pr-rerun-line.sh --single <pr-num>" >&2; exit 1; }
num="$2"

exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  log "lock busy, skipping"; echo "pr-rerun line busy" >&2; exit 1
fi
...
origin="$DIR/scratch/pr-rerun-$num-origin.md"
...
rm -f "$DIR/scratch/pr-rerun-$num-report.md"   # no stale report from a crashed round
...
HFV_SLOT="pr-rerun-$num" PR_TMPL="$TMPL" PR_TAG="rerun" PR_NUM="$num" PR_BRANCH="$branch" PR_URL="$url" PR_MID="${mid:-}" \
PR_TIMEOUT=5400 \
GH_TOKEN="$(gh auth token 2>/dev/null || true)" \
  bash "$DIR/lines/run-container.sh" --wt "$wt" --creds run-pr-main.sh
rc=$?
log "pr=$num re-run stage exit=$rc log=$(ls -t "$LOG_DIR"/run-pr-rerun-*.log 2>/dev/null | head -1)"
...
report="$DIR/scratch/pr-rerun-$num-report.md"
if [[ -s "$report" ]]; then

...
trap 'rm -f "$HFV_EXEC_SNAPSHOT"; if [[ -n "${CUR_PR:-}" && -n "${DIR:-}" ]]; then rm -f "$CUR_FILE"; docker rm -f $(docker ps -q --filter "name=-spr-rebase-$CUR_PR") >>"${LOG_DIR:-/dev/null}" 2>&1 || true; fi' EXIT
...
INST="${HFV_INST:-1}"
N_INST=1
[[ -f "$DIR/state/.scale-rebase" ]] && N_INST="$(cat "$DIR/state/.scale-rebase" 2>/dev/null)"
[[ "$N_INST" =~ ^[0-9]+$ && "$N_INST" -ge 1 ]] || N_INST=1
LOCK_FILE="$DIR/locks/pr-rebase-$INST.lock"
CUR_FILE="$DIR/state/.current-rebase-$INST"

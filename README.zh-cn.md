...
**录入。** 一个 recorder 守护进程定时扫飞书群。新报告作为不透明记录落进本地 store，附件下载到本地；截图和录屏由视觉模型转写成文字，修复者全程不需要打开任何图片。recorder 是通用组件，源是可插拔的——GitHub issue 曾经作为第二个源接入同一个 store（该线已退役，代码保留），下游只用 message_id 前缀做分发，不关心记录来自哪里。

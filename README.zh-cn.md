...
**主动找 bug。** scan 线（`hfv scan run`）审计仓库的"中间区段"文件——既不是最近几天改动的，也不是最老分位之外的——窗口是自适应的：按日历月分桶的 UCB1 选择器把批次引向历史上复现出 bug 的文件区段，连续零命中则自动扩张窗口一档。复现成功的 bug 会先报告到飞书群，然后修复、复验、经同一套交付机器开出 PR。
